...
export default class Gantt {
    ...
    setup_options(options) {
        const default_options = {
            ...
            end_date: undefined,
            highlight_dates: [],
            bar_color_fn: (task) => stringToColour(task.name)
        };
    ...
    setup_tasks(tasks) {
        ...
        this.tasks = tasks.map((task, i) => {
            ...
            task._color = this.options.bar_color_fn(task)
    ...
    bind_events() {
        ...
        $('.gantt-container').addEventListener('scroll', (e) => {
    ...
    setup_groups() {
        ...
        this.tasks.forEach((task) => {
            ...
            group_map[key].push(task)
        })
        this.group_map = group_map

        let i = 0
        let task_class_flag = true
        for (const group in group_map) {
            for (const task of group_map[group]) {
                task._index = i++
                task._row_class = task_class_flag ? '' : 'alt'
            }
            task_class_flag = !task_class_flag
        }

        this.tasks = this.tasks.sort((a, b) => a._index - b._index)
    }
    ...
    get_info_cols() {
        const info_cols = [this.options.group_by, ...this.options.info_column_keys]
        return info_cols
    }
    ...
    make_info_columns() {
        ...
        const info_cols = this.get_info_cols()
        const indexes = info_cols.map((_) => [])
        const max_index_width = info_cols.map((key) => {
        ...
        for (let task of this.tasks) {
            for (let [i, key] of info_cols.entries()) {

                const classes = []
                if (i == 0) classes.push('index')
        ...
        for (let [i, index] of indexes.entries()) {
            for (let { rect, text } of indexes[i]) {
                ...
                text.setAttribute('data-offset-x', text_x)
            }
        }
    ...
    make_grid_rows(rows_layer) {
        ...
        for (let task of this.tasks) {
            ...
            createSVG('rect', {
                ...
                width: row_width,
                height: row_height,
                class: 'grid-row ' + task._row_class,
    ...
    make_grid_header() {
        ...
        for (const [i, key] of this.get_info_cols().entries()) {
            const classes = []
            if (i == 0) classes.push('index')
            createSVG('rect', {
                x: this.options.index_xs[i],
                y: 0,
                'data-offset-x': this.options.index_xs[i],
                width: this.options.index_widths[i],
                height: header_height,
                class: ['grid-header', ...classes].join(' '),
                append_to: this.layers.index,
            });
